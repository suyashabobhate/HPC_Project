...
void atbt_kunroll_permute_kij_par(const float *__restrict__ A, const float *__restrict__ B, float *__restrict__ C, int Ni, int Nj, int Nk);
void atbt_junrollby8(const float *__restrict__ A, const float *__restrict__ B, float *__restrict__ C, int Ni, int Nj, int Nk);
  ...
  for(int version=0; version<12;version++)
  {
    ...
    for (nt=0;nt<num_cases;nt ++)
    {
     ...
     for (trial=0;trial<NTrials;trial++)
     {
      ...
      case 3: atbt_alltile16_par(A,B,C,Ni,Nj,Nk); break;
      case 4: atbt_permute_ikj_par(A,B,C,Ni,Nj,Nk); break;
      case 5: atbt_permute_kij_par(A,B,C,Ni,Nj,Nk); break;
      case 6: atbt_paralellonj_par(A,B,C,Ni,Nj,Nk); break;
      case 7: atbt_paralellonj_permute_ikj_par(A,B,C,Ni,Nj,Nk); break;
      case 8: atbt_junroll_permute_ikj_par(A,B,C,Ni,Nj,Nk); break;
      case 9: atbt_kunroll_permute_ikj_par(A,B,C,Ni,Nj,Nk); break;
      case 10: atbt_junrollby8(A,B,C,Ni,Nj,Nk); break;
      case 11: atbt_kunroll_permute_kij_par(A,B,C,Ni,Nj,Nk); break;
      }
    telapsed = omp_get_wtime()-tstart;
...
for (int l = 0; l < Ni*Nj; l++) if (fabs((C[l] - Cref[l])/Cref[l])>threshold) {printf("Error: mismatch at linearized index %d, was: %f, should be: %f for version %d\n ", l, C[l], Cref[l], version); return -1;}
    ...
    case 3: printf("Performance (Best & Worst) of parallel version for atbt_alltile16_par (in GFLOPS)"); break;
    case 4: printf("Performance (Best & Worst) of parallel version for atbt_permute_ikj_par (in GFLOPS)"); break;
    case 5: printf("Performance (Best & Worst) of parallel version for atbt_permute_kij_par (in GFLOPS)"); break;
    ...
    case 8: printf("Performance (Best & Worst) of parallel version for atbt_junroll_permute_ikj_par (in GFLOPS)"); break;
    case 9: printf("Performance (Best & Worst) of parallel version for atbt_kunroll_permute_ikj_par (in GFLOPS)"); break;
    case 10: printf("Performance (Best & Worst) of parallel version for atbt_junrollby8 (in GFLOPS)"); break;
    case 11: printf("Performance (Best & Worst) of parallel version for atbt_kunroll_permute_kij_par (in GFLOPS)"); break;
    }
