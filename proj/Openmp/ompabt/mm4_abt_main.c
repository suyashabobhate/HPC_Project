  ...
  for(int version=0; version<11;version++)
  {
    ...
    for (nt=0;nt<num_cases;nt ++)
    {
     ...
     for (trial=0;trial<NTrials;trial++)
     {
      ...
      case 3: abt_alltile16_par(A,B,C,Ni,Nj,Nk); break;
      case 4: abt_permute_ikj_par(A,B,C,Ni,Nj,Nk); break;
      case 5: abt_permute_kij_par(A,B,C,Ni,Nj,Nk); break;
...
for (int l = 0; l < Ni*Nj; l++) if (fabs((C[l] - Cref[l])/Cref[l])>threshold) {printf("Error: mismatch at linearized index %d, was: %f, should be: %f for version %d\n ", l, C[l], Cref[l], version); return -1;}
    ...
    case 3: printf("Performance (Best & Worst) of parallel version for abt_alltile16_par (in GFLOPS)"); break;
    case 4: printf("Performance (Best & Worst) of parallel version for abt_permute_ikj_par (in GFLOPS)"); break;
    case 5: printf("Performance (Best & Worst) of parallel version for abt_permute_kij_par (in GFLOPS)"); break;
